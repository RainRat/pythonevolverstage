...
if __name__ == "__main__":
  ...
  if not validate_configuration():
    print(f"{Colors.RED}Configuration check failed. Use --check for details.{Colors.ENDC}")
    sys.exit(1)

  if ALREADYSEEDED:
    if not any(os.path.exists(f"arena{i}") for i in range(LAST_ARENA + 1)):
      print(f"{Colors.RED}Error: ALREADYSEEDED is True but no arenas found. Run with --restart to seed.{Colors.ENDC}")
      sys.exit(1)
  ...
  try:
    ...
    while(True):
      ...
      status_line = f"{remaining_str} left | {bar_str} | Era {era+1} | Btl: {battle_count:,} ({bps:.1f}/s){last_battle_info}"
      # Clear line with dynamic padding based on terminal width
      term_width = shutil.get_terminal_size((80, 20)).columns
      visible_len = len(strip_ansi(status_line))
      padding = " " * max(0, term_width - visible_len - 1)
      print(f"{status_line}{padding}", end='\r')
      ...
      fl.close()
      last_battle_info = f" | {Colors.CYAN}Arena {arena}{Colors.ENDC}: {Colors.GREEN}#{winner}{Colors.ENDC} beat {Colors.RED}#{loser}{Colors.ENDC}"
    ...
    print(f"\n\n{Colors.YELLOW}Evolution interrupted by user.{Colors.ENDC}")
    print_status()
    sys.exit(0)
