...
Instruction parse_line(const std::string& line) {
    ...
    if (dot_pos != std::string::npos) {
        ...
        throw std::runtime_error("Missing modifier in instruction: " + opcode_full);
    }
    ...
    // Trim leading space
    size_t first_operand = operands_str.find_first_not_of(" \t");
    if (first_operand == std::string::npos) {
        operands_str.clear();
    } else {
        operands_str.erase(0, first_operand);
    }
        ...
        switch (instr.opcode) {
            ...
            case SEQ:
                if (check_condition(
                        ...
                        [](bool first, bool second) { return first && second; },
            ...
            case SLT:
                if (check_condition(
                        ...
                        [](const Instruction& lhs, const Instruction& rhs) {
                            return lhs.a_field < rhs.a_field && lhs.b_field < rhs.b_field;
                        })) {
                    skip = true;
                }
                break;
...
private:
    template <typename FieldComparator, typename CombineComparator, typename InstructionComparator>
    bool check_condition(
        const Instruction& src,
        const Instruction& dst,
        Modifier modifier,
        const FieldComparator& field_compare,
        const CombineComparator& combine_compare,
        const InstructionComparator& instruction_compare) const {
        switch (modifier) {
            case A:
                return field_compare(src.a_field, dst.a_field);
            case B:
                return field_compare(src.b_field, dst.b_field);
            case AB:
                return field_compare(src.a_field, dst.b_field);
            case BA:
                return field_compare(src.b_field, dst.a_field);
            case F:
                return combine_compare(
                    field_compare(src.a_field, dst.a_field),
                    field_compare(src.b_field, dst.b_field));
            case X:
                return combine_compare(
                    field_compare(src.a_field, dst.b_field),
                    field_compare(src.b_field, dst.a_field));
            case I:
                return instruction_compare(src, dst);
        }
        return false;
    }
